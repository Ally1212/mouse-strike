...
使用 Vite 前端、Colyseus 0.18 房间服务和 `multiplayer-rules.js` 提供两人实时联机。目标是服务端权威、公平可验证、弱网可恢复，同时让客户端保持鼠标跟手。
...
flowchart LR
  ...
  Client -- Colyseus SDK/WebSocket --> Server[Colyseus GameRoom]
  Server --> Rules[共享规则与配置校验]
  Server --> World[房间战斗世界/30Hz Tick]
  Server -- 15Hz Schema patch --> Client
  Client --> Render[Canvas/Three.js/HUD]
```

- `multiplayer-client.js`：按需加载 Colyseus SDK，负责连接、消息分发、输入节流和重连。
- `server.mjs`：Colyseus `GameRoom`、玩家输入、战斗世界、胜负与广播。
- `multiplayer-state.js`：Schema 状态模型与增量补丁字段。
- `multiplayer-rules.js`：房间码、配置归一化、昵称和输入校验；客户端与服务端共用。
- 房间仅保存在服务端内存；服务重启不保证恢复。

## 3. 状态机

客户端状态：`idle → connecting → lobby → ready → countdown → playing → roundEnd/matchEnd → lobby`；任何阶段可进入 `error`，用户确认后回到 `idle`。

服务端房间状态：`lobby → countdown → playing → finished`。玩家状态包含 `connected`、`ready` 和 `fighterId`；Colyseus 管理会话与 30 秒重连席位。
...
Player {
  id, nickname, fighterId, ready, connected,
  input, sim, score, roundWins, rescues, syncStrikes, pickups
}
```

Colyseus `reconnectionToken` 仅用于当前页面会话恢复，不代表账号授权；昵称统一空白折叠并截断至 16 字符。房间码使用不含易混淆字符的五位大写字母/数字。

## 5. Colyseus 房间协议

持续状态通过 Schema 增量补丁同步；客户端动作使用房间消息，失败消息为 `{ type: "error", code, message }`。

| 请求 | 关键字段 | 处理 |
| --- | --- | --- |
| SDK `create` | `nickname, mode, fighterId, config` | 创建五位房间码房间 |
| SDK `joinById` | `roomCode, nickname, fighterId` | 校验房间并加入 |
| SDK `reconnect` | `reconnectionToken` | 30 秒窗口内恢复席位 |
| `selectFighter` | `fighterId` | 校验模式可用性并取消 ready |
| `updateConfig` | `config` | 仅房主可用，归一化并取消双方 ready |
| `setReady` | `ready: boolean` | 满足条件后进入 3 秒倒计时 |
| `requestRematch` | 无 | 结算后保留房间并返回大厅 |
| `input` | `x, y, action` | 校验范围/白名单后写入最新输入 |
| `leave` | 无 | 离开房间并清理会话 |

服务端事件：`lobby`、`countdown`、`matchStart`、`roundEnd`、`matchEnd`、`error`。持续战场状态由 Schema 补丁传输，不下发重连 token 或内部判定字段。

## 6. 同步策略

- 服务端逻辑 tick 固定 30Hz，Schema patch rate 为 15Hz；每帧只保留玩家最新输入，防止输入队列积压。
...
- Colyseus 集成测试：双客户端完整建房、加入、准备、开战、状态补丁、回合、结算、重连和房主转移。
